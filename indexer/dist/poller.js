...
async function fetchListingFromChain(listingId) {
    ...
    try {
        ...
        const args = [(0, stellar_sdk_1.nativeToScVal)(listingId, { type: "u64" })];
...
async function fetchAuctionFromChain(auctionId) {
    const DUMMY_KEY = "GAAZI4TCR3TY5OJHCTJC2A4QSY6CJWJH5IAJTGKIN2ER7LBNVKOCCWN";
    try {
        if (!CONTRACT_ID)
            return null;
        const contract = new stellar_sdk_1.Contract(CONTRACT_ID);
        const args = [(0, stellar_sdk_1.nativeToScVal)(auctionId, { type: "u64" })];
        const account = await server.getAccount(DUMMY_KEY);
        const tx = new stellar_sdk_1.TransactionBuilder(account, {
            fee: stellar_sdk_1.BASE_FEE,
            networkPassphrase: process.env.STELLAR_NETWORK_PASSPHRASE || 'Test SDF Network ; September 2015',
        ...
        for (const offer of offerIdsToRevert) {
            if (offer.createdAtLedger > toLedger) {
                ...
                for (const event of events) {
                    const { eventType, ledgerSequence, data } = event;
                    const parsedData = typeof data === 'string' ? JSON.parse(data) : data;
                    if (BigInt(parsedData.offer_id) !== offer.offerId)
                        continue;
    ...
    switch (eventType) {
        ...
        case 'AUCTION_CREATED': {
            ...
            const recipients = chainAuction
                ? chainAuction.recipients.map((r) => ({
                    address: r.address.toString(),
                    percentage: Number(r.percentage)
                }))
                : [];
            await db_js_1.default.auction.upsert({
        ...
        case 'BID_PLACED': {
            await db_js_1.default.auction.update({
                where: { auctionId: listingId },
                data: {
        ...
        case 'OFFER_MADE': {
            await db_js_1.default.offer.upsert({
        ...
        case 'OFFER_ACCEPTED': {
            await db_js_1.default.offer.update({
                where: { offerId: BigInt(data.offer_id) },
                data: {
                    status: 'Accepted',
                    updatedAtLedger: ledgerSequence,
